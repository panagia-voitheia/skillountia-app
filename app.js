...
async function addVisit(){
  ...
  setTimeout(()=>loadCloud(false),800);
}
...
async function sync(type,item){
  try{await fetch(API_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'add',table:type,row:toSheetRow(type,item)})})}
...
async function deleteItem(type,id){
  ...
  save();
  try{await fetch(API_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'delete',table:type,id:id,...(item||{})})});toast('Διαγράφηκε.');setTimeout(()=>loadCloud(false),900)}
  catch(e){console.warn(e)}
}
...
function updateBadges(){
  ...
  Object.keys(map).forEach(type=>{
    const latest=Math.max(0,...(data[type]||[]).filter(x=>type==='visits'?isRealVisit(x):isRealItem(x)).map(x=>Date.parse(x.created)||0));
